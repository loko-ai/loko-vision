...
from sanic_ext.extensions.openapi.types import String, Boolean, Float, Integer
...
from config.AppConfig import REPO_PATH, PORT
...
#  - dare piu' info nel servizio evaluate; FATTO
#  - gestire fit nel caso di nome modello non piu' presente; FATTO!!!!!


# todo (11/06/2023):
#  - gestire errori dati:
#       -  dataset vuoto o con solo img non supportate; FATTO!!!!!
#       -  dataset con solo una label per il fit; FATTO!!!!!
#       -  dataset con troppi pochi sample per fit e/o evaluate?; parlarne con fulvio ---> per ora no
#       -  ;
#  - gestire pretrained con dao, per averli in memoria; FATTO!!!! ma capire se ha senso che ci metta un po' a caricarsi "veramente" in memroia
#  - rendere tutti i servizi "classici" in stream schedulando il task; FATTO!!!! ma parlarne con fulvio
#  - aggiungere volumi per keras; FATTO!!!!
#  - mostrare solo modelli nel servizio get e non anche i file nascosti (.DsSTore in mac) FATTO!!!!
#  - far sparire factory;


from utils.service_utils import check_predict_params
...
@openapi.parameter(name="optimizer", schema=String(default="adam"), location="query")
@openapi.parameter(name="epochs", schema=Integer(default=100), location="query")
@openapi.parameter(name="metrics", schema=String(default="accuracy,"), location="query")
@openapi.parameter(name="predictor_name", location="path", required=True)
@file()
async def fit(request, predictor_name):
    ...
    optimizer = request.args.get("optimizer", 'adam').lower()
    ...
    async def run_executor_train():
        result = await app.loop.run_in_executor(None, functools.partial(training_task), f, model_info, epochs,
                                                optimizer,
                                                metrics)
    ...
    return json('Model %s is fitting! Data used: %s' % (predictor_name, f.name))  # , status=200)
...
@openapi.parameter(name="multilabel", schema=Boolean(default=False), location="query")
@openapi.parameter(name="multilabel_threshold", schema=Float(default=0.5), location="query")
@openapi.parameter(name="proba_threshold", schema=Float(default=0.0001), location="query")
@openapi.parameter(name="include_probs", schema=Boolean(default=True), location="query")
@file()
async def predict(request, predictor_name):
    ...
    proba_threshold = float(request.args.get("proba_threshold", 0.0001)) if proba else None
    # proba_threshold, mlb_threshold = check_predict_params(proba, proba_threshold, multilabel, mlb_threshold)
...
async def loko_fit_model(file, args):
    ...
    try:
        ...
        logger.error(f"fitting error: {inst}")
...
async def loko_predict_model(file, args):
    ...
    proba_threshold = args.get("probability_th", 0.00) if proba else None
    ...
    mlb_threshold = args.get("multilabel_threshold", 0.5) if multilabel else None
    proba_threshold, mlb_threshold = check_predict_params(proba, proba_threshold, multilabel, mlb_threshold)
    preds_res = await app.loop.run_in_executor(None, functools.partial(predict_task), f, predictor_name, proba,
                                               multilabel, mlb_threshold, proba_threshold)
...
if __name__ == '__main__':
    app.blueprint(bp)
    app.run("0.0.0.0", port=PORT, auto_reload=False, single_process=True)
